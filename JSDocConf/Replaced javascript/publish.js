...
function linktoClass ( longName, name ) {
	let longNames = longName.split ( '~' );
	name = longNames [ 1 ] + ' ('+ longNames [ 0 ] + ')';
...
function buildNav(members) {
	...
	members.classes.sort ( 
		( a, b ) => { 
			let aSplited = a.longname.split( '~');
			let bSplited = b.longname.split( '~');
			return ( aSplited [ 1 ].toLowerCase() + '&nbsp;('+ aSplited [ 0 ].toLowerCase() + ')' > bSplited [ 1 ].toLowerCase() + '&nbsp;('+ bSplited [ 0 ].toLowerCase() + ')' ) ? 1 : -1;
		} 
	);
    ...
    nav += buildMemberNav(members.classes, 'Classes', seen, linktoClass);
